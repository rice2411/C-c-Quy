export const translations = {
  en: {
    common: {
      apply: "Apply",
      cancel: "Cancel",
      reset: "Reset"
    },
    nav: {
      dashboard: "Dashboard",
      orders: "Orders",
      inventory: "Inventory",
      customers: "Customers",
      suppliers: "Suppliers",
      users: "User Management",
      transactions: "Transactions",
      notifications: "Notifications",
      settings: "Settings",
      signOut: "Sign Out",
      newOrder: "New Order",
      soon: "SOON",
      add: "More"
    },
    header: {
      dashboardTitle: "Dashboard Overview",
      ordersTitle: "Order Management",
      inventoryTitle: "Product Inventory",
      customersTitle: "Customer Management",
      suppliersTitle: "Supplier Management",
      usersTitle: "User Management",
      transactionsTitle: "Payment Transactions",
      systemOp: "System Operational"
    },
    dashboard: {
      totalRevenue: "Total Revenue",
      totalOrders: "Total Orders",
      pending: "Pending",
      ingredientCost: "Ingredient Cost",
      recentOrders: "Recent Orders",
      recentTransactions: "Recent Transactions",
      recentUsers: "Recent Logins",
      revenueTrend: "Revenue Trend",
      fromLastWeek: "from last week",
      newToday: "new today",
      requiresAttention: "Requires attention",
      fromYesterday: "from yesterday",
      aiTitle: "Gemini Insights",
      analyzing: "Analyzing data...",
      updateAnalysis: "Update Analysis",
      generateBrief: "Generate Daily Brief",
      aiPlaceholder: "Generate a real-time AI analysis of your current order trends, potential risks, and operational suggestions using Google Gemini.",
      filterWeek: "Week",
      filterMonth: "Month",
      filterYear: "Year"
    },
    orders: {
      recent: "Recent Orders",
      searchPlaceholder: "Search ID or Customer...",
      filterProductPlaceholder: "Filter product...",
      filters: "Filters",
      fromDate: "From date",
      toDate: "To date",
      startDate: "Start Month",
      endDate: "End Month",
      allMonths: "All Months",
      allStatus: "All Status",
      noOrders: "No orders found.",
      noOrdersCriteria: "No orders found matching your criteria.",
      createFirst: "Create First Order",
      showing: "Showing",
      of: "of",
      viewDetails: "View Details",
      tableId: "ID",
      tableDate: "Date",
      tableDeliveryDate: "Delivery date",
      dateType: "Date Type",
      orderDateLabel: "Order Date",
      deliveryDateLabel: "Delivery Date",
      tableCreatedUpdated: "Created / Updated",
      tableCustomer: "Customer",
      tableProduct: "Product",
      tableStatus: "Status",
      tableTotal: "Total",
      tableActions: "Actions",
      labelCreated: "Created",
      labelUpdated: "Updated",
      statusLabels: {
        PENDING: "Pending",
        PROCESSING: "Processing",
        DELIVERED: "Delivered",
        CANCELLED: "Cancelled",
        RETURNED: "Returned"
      },
      paymentStatusLabels: {
        PAID: "Paid",
        UNPAID: "Unpaid",
        REFUNDED: "Refunded"
      },
      delete: "Delete Order",
      confirmDelete: "Are you sure you want to delete this order? This action cannot be undone.",
      editDeniedCollaborator: "You can only edit orders you created.",
      refresh: "Refresh",
      refreshSuccess: "Orders refreshed successfully",
      refreshError: "Failed to refresh orders",
      exportCsv: "Export CSV",
      exportTitle: "Export Order Data",
      exportCurrentMonth: "Current Month",
      exportMonth: "Select Month",
      exportAllTime: "All Time",
      exportCustom: "Custom Range",
      exportStart: "Start Date",
      exportEnd: "End Date",
      exportCancel: "Cancel",
      exportConfirm: "Export",
      stats: {
        scopeHint: "Across all orders in the system",
        cancelledOrders: "Cancelled orders",
        cancelledHint: "Orders marked as cancelled",
        unpaidOrders: "Unpaid orders",
        unpaidHint: "Awaiting payment (excludes cancelled)",
      },
    },
    notifications: {
      title: "Notification Management",
      subtitle: "Send notifications by group — Zalo, orders, and more",
      groups: {
        zalo: "Zalo",
        zaloDesc: "Custom messages to your Zalo group",
        customers: "Customers",
        customersDesc: "Customer-related notifications",
        orders: "Orders",
        ordersDesc: "Notifications based on order status or delivery date",
        transactions: "Transactions",
        transactionsDesc: "Payment and transaction alerts",
        empty: "No notification types in this group yet.",
      },
      unpaidOrders: "Unpaid Orders",
      unpaidOrdersDesc: "Send notification about all unpaid orders",
      pendingOrders: "Pending Orders",
      pendingOrdersDesc: "Send notification about orders that need to be processed (not delivered or cancelled)",
      deliveryOrders: "Delivery Due Orders",
      deliveryOrdersDesc: "Send notification about orders that need to be delivered on a specific date",
      customMessage: "Custom Message",
      customMessageDesc: "Send a custom message to the Zalo group",
      send: "Send Notification",
      sending: "Sending...",
      sentSuccess: "Notification sent successfully",
      sendError: "Failed to send notification",
      noUnpaidOrders: "No unpaid orders found",
      noPendingOrders: "No pending orders found",
      noDeliveryOrders: "No orders need to be delivered on this date",
      selectDeliveryDate: "Select delivery date",
      enterMessage: "Please enter a message",
      messagePlaceholder: "Enter your custom message here...",
      foundOrders: "Found {count} orders",
      selectedTypeInfo: "Will send notification about {count} {type}"
    },
    inventory: {
      title: "Products",
      addProduct: "Add Product",
      searchPlaceholder: "Search products...",
      noProducts: "No products found.",
      createFirst: "Add your first product",
      productsTab: "Products",
      ingredientsTab: "Ingredients / Materials",
      recipesTab: "Recipes",
      ingredientsPlaceholder: "Ingredient/Material tracking has been disabled in this version.",
      recipesPlaceholder: "Recipes module is coming soon.",
      formTitleAdd: "Add New Product",
      formTitleEdit: "Edit Product",
      name: "Product Name",
      price: "Price (VND)",
      category: "Category",
      image: "Image URL",
      description: "Description",
      status: "Status",
      active: "Active",
      inactive: "Inactive",
      deleteConfirm: "Are you sure you want to delete this product?"
    },
    ingredients: {
      title: "Ingredients / Materials",
      quantity: "Quantity",
      add: "Add Ingredient/Material",
      searchPlaceholder: "Search ingredients or materials...",
      noData: "No ingredients or materials found.",
      createFirst: "Add your first ingredient/material",
      noSupplier: "No supplier",
      deleteConfirm: "Are you sure you want to delete this ingredient?",
      tabDetails: "Ingredient Details",
      tabHistory: "History",
      historyEmpty: "No history for this ingredient yet.",
      noSearchResults: "No search results found.",
      stockLabel: "Stock",
      outOfStock: "Out of Stock",
      lowStock: "Low Stock",
      loading: "Loading...",
      items: "item",
      itemsPlural: "items",
      currentQuantity: "Current Quantity",
      initialQuantity: "Initial",
      totalImport: "Import",
      totalUsage: "Usage",
      totalImportPrice: "Total Import Price",
      totalImportWeight: "Total Import Weight",
      importCount: "Import Count",
      times: "times",
      form: {
        addTitle: "Add Ingredient",
        editTitle: "Edit Ingredient",
        basicInfo: "Basic information",
        priceInfo: "Price & supplier",
        name: "Ingredient Name",
        namePlaceholder: "e.g. Flour, Sugar",
        subtitle: "Track core info for each ingredient",
        type: "Type",
        types: {
          base: "Base",
          flavor: "Flavor",
          topping: "Topping",
          decoration: "Decoration",
          material: "Material",
          BASE: "Base",
          FLAVOR: "Flavor",
          TOPPING: "Topping",
          DECORATION: "Decoration",
          MATERIAL: "Material"
        },
        quantity: "Quantity",
        quantityPlaceholder: "e.g. 500",
        weight: "Weight",
        initialQuantity: "Initial quantity",
        initialQuantityPlaceholder: "e.g. 0",
        fromQuantity: "From quantity",
        fromQuantityPlaceholder: "e.g. 0",
        importQuantity: "Import quantity",
        usageQuantity: "Usage quantity",
        usageQuantityPlaceholder: "e.g. 100",
        unit: "Unit",
        unitPrice: "Unit price",
        unitPiece: "piece",
        supplier: "Supplier",
        supplierPlaceholder: "Select supplier",
        note: "Note",
        notePlaceholder: "Additional notes...",
        historyTitle: "Create record",
        historyEditTitle: "Edit History",
        historyType: "Record type",
        historyTypeImport: "Stock in",
        historyTypeUsage: "Usage",
        historyDate: "Record date",
        historyPrice: "Price",
        historyPricePlaceholder: "e.g. 120000",
        productWeight: "Product weight",
        productWeightPlaceholderPiece: "e.g. 50",
        productWeightPlaceholderGram: "e.g. 1",
        totalWeight: "Total weight",
        totalWeightLabel: "✓ Total weight:",
        importQuantityPlaceholder: "e.g. 2, 3, 4...",
        importQuantityHint: "Quantity × {productWeight}g/{unit} = Total weight",
        importDate: "Import date",
        totalAmount: "Total amount",
        searchHistory: "Search history...",
        historyQuantityBefore: "Before / after",
        historyDelete: "Delete",
        historyAdd: "Stock in",
        save: "Save Ingredient",
        errors: {
          nameRequired: "Ingredient name is required",
          typeRequired: "Ingredient type is required",
          quantityRequired: "Quantity must be greater than 0",
          productWeightRequired: "Product weight must be greater than 0",
          toQuantityMustBeGreater: "To quantity must be greater than from quantity",
          supplierRequired: "Supplier is required for stock in",
          usageExceedsQuantity: "Usage quantity cannot exceed current quantity",
          saveFailed: "Unable to save ingredient"
        }
      }
    },
    recipes: {
      title: "Recipes",
      add: "Add Recipe",
      addBase: "Add Base Recipe",
      addFull: "Add Full Recipe",
      all: "All",
      searchPlaceholder: "Search recipes...",
      noData: "No recipes found.",
      createFirst: "Add your first recipe",
      loading: "Loading...",
      ingredients: "ingredients",
      servings: "servings",
      more: "more",
      delete: "Delete Recipe",
      deleteConfirmTitle: "Confirm Delete Recipe",
      deleteConfirmMessage: "Are you sure you want to delete recipe \"{name}\"? This action cannot be undone.",
      deleteSuccess: "Recipe deleted successfully",
      deleteError: "Failed to delete recipe",
      form: {
        addTitle: "Add Recipe",
        editTitle: "Edit Recipe",
        addBaseRecipe: "Add Base Recipe",
        editBaseRecipe: "Edit Base Recipe",
        baseRecipeSubtitle: "Create a base recipe from basic ingredients",
        addFullRecipe: "Add Full Recipe",
        editFullRecipe: "Edit Full Recipe",
        fullRecipeSubtitle: "Create a full recipe from base recipe + flavor + topping",
        subtitle: "Create a recipe from ingredients",
        name: "Recipe Name",
        namePlaceholder: "e.g. Chocolate Cake",
        description: "Description",
        descriptionPlaceholder: "Brief description of the recipe...",
        ingredients: "Ingredients",
        addIngredientPlaceholder: "Search and add ingredients...",
        instructions: "Instructions",
        instructionsPlaceholder: "Step-by-step instructions...",
        yield: "Yield",
        yieldUnit: "Yield Unit",
        yieldUnitPlaceholder: "e.g. pieces, servings",
        useCalculatedYield: "Use calculated",
        outputQuantity: "Output Quantity",
        outputQuantityOfRecipe: "Recipe Output Quantity",
        wasteRate: "Waste Rate",
        finalQuantity: "Final Quantity (After Waste)",
        calculatedFinalQuantity: "Product Quantity",
        available: "Available",
        recipeType: "Recipe Type",
        baseRecipe: "Base Recipe",
        fullRecipe: "Full Recipe",
        baseRecipeSelect: "Select Base Recipe",
        selectBaseRecipe: "Select a base recipe...",
        availableIngredients: "Available",
        selectedIngredients: "Selected",
        selected: "selected",
        allIngredientsAdded: "All ingredients added",
        allAdded: "All added",
        none: "None",
        noIngredientsSelected: "Click to add ingredients",
        outOfStock: "Out of stock",
        setQuantity: "Set Quantity",
        quantity: "Quantity",
        save: "Save Recipe",
        tabEdit: "Edit",
        tabCalculation: "Calculation",
        calculationQuantity: "Recipe Quantity",
        calculationProductQuantity: "Product Quantity to Produce",
        noIngredientsForCalculation: "Please add ingredients to the recipe before calculating",
        enterQuantityForCalculation: "Please enter recipe quantity",
        enterProductQuantityForCalculation: "Please enter product quantity to produce",
        setOutputQuantityFirst: "Please set output quantity in edit tab first",
        allIngredientsSufficient: "All ingredients sufficient",
        someIngredientsInsufficient: "Some ingredients insufficient",
        ingredientName: "Ingredient",
        required: "Required",
        status: "Status",
        sufficient: "Sufficient",
        shortage: "Shortage",
        requiredRecipeCount: "Required Recipe Count",
        maxPossibleRecipeCount: "Maximum Possible Recipe Count",
        maxPossibleProductQuantity: "Maximum Possible Product Quantity",
        tabCost: "Cost",
        noIngredientsForCost: "Please add ingredients to the recipe before viewing cost",
        quantityInRecipe: "Quantity in Recipe",
        averagePrice: "Average Price",
        totalCostForQuantity: "Total Cost for This Quantity",
        totalRecipeCost: "Total Recipe Cost",
        costPerProduct: "Cost Per Product",
        for: "for",
        products: "products",
        noPriceData: "No price data available",
        fromBaseRecipe: "From base recipe",
        additionalIngredients: "Additional Ingredients",
        baseRecipeCost: "Base Recipe Cost",
        additionalCost: "Additional Ingredients Cost",
        errors: {
          nameRequired: "Recipe name is required",
          ingredientsRequired: "At least one ingredient is required",
          quantityRequired: "All ingredient quantities must be greater than 0",
          baseRecipeRequired: "Please select a base recipe for full recipe",
          saveFailed: "Unable to save recipe"
        }
      }
    },
    customers: {
      title: "Customers",
      pageSubtitle: "Directory synced with orders — search by name or phone.",
      add: "Add Customer",
      search: "Search by name or phone…",
      noData: "No customers found.",
      createFirst: "Add your first customer",
      searchEmpty: "No customers match this search.",
      listToolbarHint: "{{filtered}} of {{total}} shown",
      statsPillTotal: "Total {{n}}",
      statsPillActive: "{{n}} with order history",
      namePlaceholder: "e.g. Jane Doe",
      phonePlaceholder: "e.g. 0901234567",
      phoneEmpty: "Phone number is empty",
      phoneEmptyBadge: "No phone",
      loadingHint: "Loading customers…",
      phoneBadge: {
        invalid: "Invalid mobile number",
        viettel: "Viettel",
        vina: "VinaPhone",
        mobi: "MobiFone",
        vietnamobile: "Vietnamobile",
        gmobile: "Gmobile",
      },
      table: {
        name: "Customer",
        contact: "Contact Info",
        location: "Location",
        totalProducts: "Total Products",
        actions: "Actions"
      },
      form: {
        addTitle: "Add New Customer",
        editTitle: "Edit Customer",
        sectionTitle: "Customer details",
        name: "Full Name",
        email: "Email",
        phone: "Phone Number",
        address: "Address",
        city: "City",
        country: "Country",
        save: "Save Customer",
        errors: {
          nameRequired: "Name is required",
          saveFailed: "Could not save customer",
        },
      },
      delete: {
        title: "Delete Customer",
        confirm: "Are you sure you want to delete this customer? This action cannot be undone."
      },
      detail: {
        contactSection: "Contact",
        ordersHeading: "Orders ({{n}})",
        noOrders: "No orders linked to this phone number yet.",
        itemsLine: "{{n}} products in order",
        paymentPending: "Not paid yet",
        paymentDateLabel: "Payment recorded at",
        paymentAtHint: "Recorded when status was set to paid (no separate payment timestamp).",
      }
    },
    suppliers: {
      title: "Suppliers",
      add: "Add Supplier",
      search: "Search suppliers...",
      noData: "No suppliers found.",
      createFirst: "Add your first supplier",
      form: {
        addTitle: "Add New Supplier",
        editTitle: "Edit Supplier",
        name: "Supplier Name",
        namePlaceholder: "e.g. Flour Co., Sugar Ltd.",
        type: "Supplier Type",
        types: {
          grocery: "Grocery",
          tiktok: "TikTok",
          shopee: "Shopee",
          GROCERY: "Grocery",
          TIKTOK: "TikTok",
          SHOPEE: "Shopee"
        },
        contactName: "Contact Person",
        contactPlaceholder: "Contact name",
        phone: "Phone",
        email: "Email",
        address: "Address",
        addressPlaceholder: "Street, City",
        note: "Note",
        notePlaceholder: "Additional notes...",
        save: "Save Supplier",
        errors: {
          nameRequired: "Supplier name is required",
          saveFailed: "Unable to save supplier"
        }
      },
      delete: {
        title: "Delete Supplier",
        confirm: "Are you sure you want to delete this supplier? This action cannot be undone."
      },
      table: {
        name: "Supplier",
        contact: "Contact",
        actions: "Actions"
      }
    },
    users: {
      title: "User Management",
      searchPlaceholder: "Search by name, email...",
      noUsers: "No users found.",
      stats: {
        total: "Total Users",
        pending: "Pending",
        active: "Active",
        inactive: "Inactive"
      },
      status: {
        pending: "Pending",
        active: "Active",
        inactive: "Inactive"
      },
      role: {
        superAdmin: "Super Admin",
        admin: "Admin",
        colaborator: "Colaborator"
      },
      filter: {
        all: "All Status"
      },
      table: {
        user: "User",
        customName: "Custom Name",
        role: "Role",
        status: "Status",
        lastLogin: "Last Login",
        actions: "Actions"
      },
      actions: {
        approve: "Approve",
        activate: "Activate",
        deactivate: "Deactivate",
        edit: "Edit",
        cancel: "Cancel",
        save: "Save"
      },
      messages: {
        statusUpdated: "User status updated successfully",
        customNameUpdated: "Custom name updated successfully",
        roleUpdated: "User role updated successfully",
        loadError: "Failed to load users",
        updateStatusError: "Failed to update status",
        updateCustomNameError: "Failed to update custom name",
        updateRoleError: "Failed to update role"
      }
    },
    transactions: {
      title: "Transactions",
      searchPlaceholder: "Search order #, content...",
      noData: "No transactions found.",
      refresh: "Refresh",
      refreshSuccess: "Transactions refreshed successfully",
      refreshError: "Failed to refresh transactions",
      loadError: "Failed to load transactions",
      totalIn: "Total In",
      totalOut: "Total Out",
      netAmount: "Net Amount",
      filterAll: "All",
      filterIn: "Incoming",
      filterOut: "Outgoing",
      results: "results",
      dateRange: "Date range",
      from: "From",
      to: "To",
      table: {
        date: "Date",
        amount: "Amount",
        orderNumber: "Order Ref",
        content: "Content",
        gateway: "Gateway",
        account: "Account"
      }
    },
    form: {
      createTitle: "Create New Order",
      editTitle: "Edit Order",
      customerDetails: "Customer Details",
      customerName: "Customer Name",
      phone: "Phone Number",
      address: "Delivery Address",
      orderInfo: "Order Info",
      productType: "Product Type",
      customName: "Tên sản phẩm",
      customPlaceholder: "e.g. Special Cake",
      quantity: "Quantity",
      unitPrice: "Unit Price (VND)",
      shippingCost: "Shipping Cost (VND)",
      totalEstimate: "Total Estimate",
      status: "Status",
      note: "Note",
      cancel: "Cancel",
      save: "Save",
      edit: "Edit",
      saving: "Saving...",
      saveProduct: "Save Product"
    },
    detail: {
      orderId: "Order #",
      placedOn: "Placed on",
      tabDetails: "Order Details",
      tabAi: "AI Assistant",
      history: "History",
      historyEmptyAll: "No history available",
      customer: "Customer",
      customerName: "Customer Name",
      email: "Email",
      phone: "Phone",
      shippingAddress: "Shipping Address",
      items: "Items",
      subtotal: "Subtotal",
      shipping: "Shipping",
      total: "Total",
      fulfillment: "Fulfillment",
      tracking: "Tracking Number",
      payment: "Payment Status",
      paymentMethod: "Payment Method",
      transactionNumber: "Transaction Number",
      notAssigned: "Not assigned",
      note: "Note",
      noNote: "No note provided",
      close: "Close",
      edit: "Edit Order",
      aiIntro: "Use advanced AI to analyze this order, draft communications, or assess risk factors instantly.",
      draftEmail: "Draft Email",
      riskCheck: "Risk Check",
      summarize: "Summarize",
      copyClipboard: "Copy to Clipboard",
      consulting: "Consulting Gemini..."
    },
    paymentMethod: {
      label: "Payment Method",
      cash: "Cash",
      banking: "Banking"
    },
    qr: {
      sectionTitle: "Payment QR",
      title: "Bank Transfer (VietQR)",
      bank: "Bank",
      account: "Account",
      accountName: "Name",
      amount: "Amount",
      content: "Content",
      instruction: "Scan with any banking app to pay."
    },
    login: {
      welcome: "Welcome back! Please sign in to continue.",
      googleButton: "Sign in with Google",
      currentAccount: "Current Account",
      useCurrentAccount: "Continue with this account",
      switchAccount: "Switch Account / Add New",
      recentAccounts: "Recent Accounts",
      accountRemoved: "Account removed from history",
      removeAccount: "Remove account"
    }
  },
  vi: {
    common: {
      apply: "Áp dụng",
      cancel: "Hủy",
      reset: "Đặt lại"
    },
    nav: {
      dashboard: "Tổng quan",
      orders: "Đơn hàng",
      inventory: "Kho hàng",
      customers: "Khách hàng",
      suppliers: "Nhà cung cấp",
      users: "Quản lý người dùng",
      transactions: "Giao dịch",
      notifications: "Quản lý thông báo",
      settings: "Cài đặt",
      signOut: "Đăng xuất",
      newOrder: "Tạo đơn",
      soon: "SẮP RA",
      add: "Thêm"
    },
    header: {
      dashboardTitle: "Tổng quan hệ thống",
      ordersTitle: "Quản lý đơn hàng",
      inventoryTitle: "Kho sản phẩm",
      customersTitle: "Quản lý khách hàng",
      suppliersTitle: "Quản lý nhà cung cấp",
      transactionsTitle: "Lịch sử giao dịch",
      usersTitle: "Quản lý người dùng",
      systemOp: "Hệ thống ổn định"
    },
    dashboard: {
      totalRevenue: "Tổng doanh thu",
      totalOrders: "Tổng đơn hàng",
      pending: "Chờ xử lý",
      ingredientCost: "Tiền tiêu nguyên liệu",
      recentOrders: "Đơn hàng gần đây",
      recentTransactions: "Giao dịch gần đây",
      recentUsers: "Người dùng đăng nhập gần đây",
      revenueTrend: "Biểu đồ doanh thu",
      fromLastWeek: "so với tuần trước",
      newToday: "đơn mới hôm nay",
      requiresAttention: "Cần chú ý",
      fromYesterday: "so với hôm qua",
      aiTitle: "Phân tích Gemini",
      analyzing: "Đang phân tích...",
      updateAnalysis: "Cập nhật phân tích",
      generateBrief: "Tạo báo cáo ngày",
      aiPlaceholder: "Sử dụng AI để phân tích xu hướng bán hàng, rủi ro tiềm ẩn và đề xuất vận hành theo thời gian thực.",
      filterWeek: "Tuần",
      filterMonth: "Tháng",
      filterYear: "Năm"
    },
    orders: {
      recent: "Đơn hàng gần đây",
      searchPlaceholder: "Tìm ID hoặc Khách hàng...",
      filterProductPlaceholder: "Lọc theo sản phẩm...",
      filters: "Bộ lọc",
      fromDate: "Từ ngày",
      toDate: "Đến ngày",
      startDate: "Tháng bắt đầu",
      endDate: "Tháng kết thúc",
      allMonths: "Tất cả các tháng",
      allStatus: "Tất cả trạng thái",
      noOrders: "Chưa có đơn hàng nào.",
      noOrdersCriteria: "Không tìm thấy đơn hàng phù hợp.",
      createFirst: "Tạo đơn đầu tiên",
      showing: "Hiển thị",
      of: "trên",
      viewDetails: "Xem chi tiết",
      tableId: "ID",
      tableDate: "Ngày",
      tableDeliveryDate: "Ngày giao",
      dateType: "Loại ngày",
      orderDateLabel: "Ngày tạo đơn",
      deliveryDateLabel: "Ngày giao",
      tableCreatedUpdated: "Tạo / Sửa",
      tableCustomer: "Khách hàng",
      tableProduct: "Sản phẩm",
      tableStatus: "Trạng thái",
      tableTotal: "Tổng tiền",
      tableActions: "Thao tác",
      labelCreated: "Tạo",
      labelUpdated: "Sửa",
      statusLabels: {
        PENDING: "Chờ xử lý",
        PROCESSING: "Đang xử lý",
        DELIVERED: "Đã giao",
        CANCELLED: "Đã hủy",
        RETURNED: "Trả hàng"
      },
      paymentStatusLabels: {
        PAID: "Đã thanh toán",
        UNPAID: "Chưa thanh toán",
        REFUNDED: "Đã hoàn trả"
      },
      delete: "Xóa đơn hàng",
      confirmDelete: "Bạn có chắc chắn muốn xóa đơn hàng này không? Hành động này không thể hoàn tác.",
      editDeniedCollaborator: "Bạn chỉ được chỉnh sửa các đơn do chính bạn tạo.",
      refresh: "Làm mới",
      refreshSuccess: "Đã làm mới danh sách đơn hàng thành công",
      refreshError: "Không thể làm mới danh sách đơn hàng",
      exportCsv: "Xuất CSV",
      exportTitle: "Xuất dữ liệu đơn hàng",
      exportCurrentMonth: "Tháng này",
      exportMonth: "Chọn tháng",
      exportAllTime: "Tất cả",
      exportCustom: "Tùy chỉnh",
      exportStart: "Từ ngày",
      exportEnd: "Đến ngày",
      exportCancel: "Hủy",
      exportConfirm: "Xuất file",
      stats: {
        scopeHint: "Trên toàn bộ đơn trong hệ thống",
        cancelledOrders: "Đơn bị hủy",
        cancelledHint: "Đơn đã đánh dấu hủy",
        unpaidOrders: "Đơn chưa thanh toán",
        unpaidHint: "Chờ thanh toán (không tính đơn đã hủy)",
      },
    },
    notifications: {
      title: "Quản lý thông báo",
      subtitle: "Gửi thông báo theo nhóm — Zalo, đơn hàng, v.v.",
      groups: {
        zalo: "Zalo",
        zaloDesc: "Tin tùy chỉnh gửi nhóm Zalo",
        customers: "Khách hàng",
        customersDesc: "Thông báo liên quan khách hàng",
        orders: "Đơn hàng",
        ordersDesc: "Thông báo theo trạng thái hoặc ngày giao đơn",
        transactions: "Giao dịch",
        transactionsDesc: "Cảnh báo thanh toán và giao dịch",
        empty: "Chưa có loại thông báo trong nhóm này.",
      },
      unpaidOrders: "Đơn hàng chưa thanh toán",
      unpaidOrdersDesc: "Gửi thông báo về tất cả đơn hàng chưa thanh toán",
      pendingOrders: "Đơn hàng cần xử lý",
      pendingOrdersDesc: "Gửi thông báo về các đơn hàng cần xử lý (chưa giao và chưa hủy)",
      deliveryOrders: "Đơn hàng cần giao",
      deliveryOrdersDesc: "Gửi thông báo về đơn hàng cần giao trong ngày cụ thể",
      customMessage: "Thông báo tùy chỉnh",
      customMessageDesc: "Gửi thông báo tùy chỉnh đến nhóm Zalo",
      send: "Gửi thông báo",
      sending: "Đang gửi...",
      sentSuccess: "Gửi thông báo thành công",
      sendError: "Gửi thông báo thất bại",
      noUnpaidOrders: "Không có đơn hàng chưa thanh toán",
      noPendingOrders: "Không có đơn hàng cần xử lý",
      noDeliveryOrders: "Không có đơn hàng cần giao vào ngày này",
      selectDeliveryDate: "Chọn ngày giao",
      enterMessage: "Vui lòng nhập nội dung thông báo",
      messagePlaceholder: "Nhập nội dung thông báo tùy chỉnh của bạn...",
      foundOrders: "Tìm thấy {count} đơn hàng",
      selectedTypeInfo: "Sẽ gửi thông báo về {count} {type}"
    },
    inventory: {
      title: "Sản phẩm",
      addProduct: "Thêm sản phẩm",
      searchPlaceholder: "Tìm kiếm sản phẩm...",
      noProducts: "Chưa có sản phẩm nào.",
      createFirst: "Thêm sản phẩm đầu tiên",
      productsTab: "Sản phẩm",
      ingredientsTab: "Nguyên / Vật liệu",
      recipesTab: "Công thức",
      ingredientsPlaceholder: "Theo dõi nguyên / vật liệu đã được tạm ẩn ở phiên bản này.",
      recipesPlaceholder: "Tính năng công thức sẽ sớm ra mắt.",
      formTitleAdd: "Thêm sản phẩm mới",
      formTitleEdit: "Sửa sản phẩm",
      name: "Tên sản phẩm",
      price: "Giá (VND)",
      category: "Danh mục",
      image: "Link ảnh",
      description: "Mô tả",
      status: "Trạng thái",
      active: "Hoạt động",
      inactive: "Ngừng bán",
      deleteConfirm: "Bạn có chắc chắn muốn xóa sản phẩm này không?"
    },
    ingredients: {
      title: "Nguyên / Vật liệu",
      quantity: "Số lượng",
      add: "Thêm nguyên / vật liệu",
      searchPlaceholder: "Tìm nguyên / vật liệu...",
      noData: "Không có nguyên / vật liệu nào.",
      createFirst: "Thêm nguyên / vật liệu đầu tiên",
      noSupplier: "Chưa có nhà cung cấp",
      deleteConfirm: "Bạn có chắc chắn muốn xóa nguyên / vật liệu này không?",
      tabDetails: "Chi tiết nguyên / vật liệu",
      tabHistory: "Lịch sử",
      historyEmpty: "Chưa có lịch sử cho nguyên liệu này.",
      noSearchResults: "Không tìm thấy kết quả.",
      stockLabel: "Tồn kho",
      outOfStock: "Hết hàng",
      lowStock: "Sắp hết",
      loading: "Đang tải...",
      items: "mục",
      itemsPlural: "mục",
      currentQuantity: "Số lượng hiện tại",
      initialQuantity: "Ban đầu",
      totalImport: "Nhập",
      totalUsage: "Sử dụng",
      totalImportPrice: "Tổng giá nhập",
      totalImportWeight: "Tổng khối lượng",
      importCount: "Số lần nhập",
      times: "lần",
      form: {
        addTitle: "Thêm nguyên liệu",
        editTitle: "Sửa nguyên liệu",
        basicInfo: "Thông tin cơ bản",
        priceInfo: "Giá & nhà cung cấp",
        name: "Tên nguyên liệu",
        namePlaceholder: "VD: Bột mì, Đường",
        subtitle: "Theo dõi 3 thông tin chính của nguyên liệu",
        type: "Loại",
        types: {
          base: "Cơ bản",
          flavor: "Hương vị",
          topping: "Topping",
          decoration: "Trang trí",
          material: "Vật liệu",
          BASE: "Cơ bản",
          FLAVOR: "Hương vị",
          TOPPING: "Topping",
          DECORATION: "Trang trí",
          MATERIAL: "Vật liệu"
        },
        quantity: "Khối lượng/Số lượng",
        quantityPlaceholder: "VD: 500",
        weight: "Khối lượng",
        initialQuantity: "Số lượng ban đầu",
        initialQuantityPlaceholder: "VD: 0",
        fromQuantity: "Số lượng từ",
        fromQuantityPlaceholder: "VD: 0",
        importQuantity: "Số lượng nhập",
        usageQuantity: "Số lượng sử dụng",
        usageQuantityPlaceholder: "VD: 100",
        unit: "Đơn vị",
        unitPiece: "cái",
        unitPrice: "Đơn giá",
        supplier: "Nhà cung cấp",
        supplierPlaceholder: "Chọn nhà cung cấp",
        note: "Ghi chú",
        notePlaceholder: "Thêm ghi chú...",
        historyTitle: "Tạo ghi nhận",
        historyEditTitle: "Chỉnh sửa lịch sử",
        historyType: "Loại ghi nhận",
        historyTypeImport: "Nhập hàng",
        historyTypeUsage: "Sử dụng",
        historyDate: "Ngày ghi nhận",
        historyPrice: "Giá nhập",
        historyPricePlaceholder: "VD: 120000",
        productWeight: "Khối lượng sản phẩm",
        productWeightPlaceholderPiece: "VD: 50",
        productWeightPlaceholderGram: "VD: 1",
        totalWeight: "Tổng khối lượng",
        totalWeightLabel: "✓ Tổng khối lượng:",
        importQuantityPlaceholder: "VD: 2, 3, 4...",
        importQuantityHint: "Số lượng × {productWeight}g/{unit} = Tổng khối lượng",
        importDate: "Ngày nhập",
        totalAmount: "Tổng tiền",
        searchHistory: "Tìm kiếm lịch sử...",
        historyQuantityBefore: "Trước / sau",
        historyDelete: "Xóa",
        historyAdd: "Nhập hàng",
        save: "Lưu nguyên liệu",
        errors: {
          nameRequired: "Tên nguyên liệu là bắt buộc",
          typeRequired: "Cần chọn loại nguyên liệu",
          quantityRequired: "Khối lượng/Số lượng phải lớn hơn 0",
          productWeightRequired: "Khối lượng sản phẩm phải lớn hơn 0",
          toQuantityMustBeGreater: "Số lượng đến phải lớn hơn số lượng từ",
          supplierRequired: "Cần chọn nhà cung cấp khi nhập hàng",
          usageExceedsQuantity: "Số lượng sử dụng không được vượt quá số lượng hiện có",
          saveFailed: "Không thể lưu nguyên liệu"
        }
      }
    },
    recipes: {
      title: "Công thức",
      add: "Thêm công thức",
      addBase: "Thêm công thức nền",
      addFull: "Thêm công thức bánh",
      all: "Tất cả",
      searchPlaceholder: "Tìm kiếm công thức...",
      noData: "Không có công thức nào.",
      createFirst: "Thêm công thức đầu tiên",
      loading: "Đang tải...",
      ingredients: "nguyên liệu",
      servings: "phần",
      more: "khác",
      delete: "Xóa công thức",
      deleteConfirmTitle: "Xác nhận xóa công thức",
      deleteConfirmMessage: "Bạn có chắc chắn muốn xóa công thức \"{name}\"? Hành động này không thể hoàn tác.",
      deleteSuccess: "Đã xóa công thức thành công",
      deleteError: "Không thể xóa công thức",
      form: {
        addTitle: "Thêm công thức",
        editTitle: "Chỉnh sửa công thức",
        addBaseRecipe: "Thêm công thức nền",
        editBaseRecipe: "Sửa công thức nền",
        baseRecipeSubtitle: "Tạo công thức nền từ nguyên liệu cơ bản",
        addFullRecipe: "Thêm công thức bánh",
        editFullRecipe: "Sửa công thức bánh",
        fullRecipeSubtitle: "Tạo công thức bánh từ công thức nền + hương vị + topping",
        subtitle: "Tạo công thức từ nguyên liệu",
        name: "Tên công thức",
        namePlaceholder: "VD: Bánh chocolate",
        description: "Mô tả",
        descriptionPlaceholder: "Mô tả ngắn về công thức...",
        ingredients: "Nguyên liệu",
        addIngredientPlaceholder: "Tìm và thêm nguyên liệu...",
        instructions: "Hướng dẫn",
        instructionsPlaceholder: "Hướng dẫn từng bước...",
        yield: "Sản lượng",
        yieldUnit: "Đơn vị sản lượng",
        yieldUnitPlaceholder: "VD: cái, phần",
        useCalculatedYield: "Dùng giá trị tính toán",
        outputQuantity: "Số lượng thành phẩm",
        outputQuantityOfRecipe: "Số lượng thành phẩm của công thức",
        wasteRate: "Tỉ lệ hao hụt",
        finalQuantity: "Số lượng bán cuối cùng",
        calculatedFinalQuantity: "Số lượng thành phẩm",
        available: "Còn lại",
        recipeType: "Loại công thức",
        baseRecipe: "Công thức nền",
        fullRecipe: "Công thức bánh",
        baseRecipeSelect: "Chọn công thức nền",
        selectBaseRecipe: "Chọn công thức nền...",
        availableIngredients: "Có sẵn",
        selectedIngredients: "Đã chọn",
        selected: "đã chọn",
        allIngredientsAdded: "Đã thêm tất cả nguyên liệu",
        allAdded: "Đã thêm hết",
        none: "Chưa có",
        noIngredientsSelected: "Click để thêm nguyên liệu",
        outOfStock: "Hết hàng",
        setQuantity: "Nhập số lượng",
        quantity: "Số lượng",
        save: "Lưu công thức",
        tabEdit: "Chỉnh sửa",
        tabCalculation: "Tính toán",
        calculationQuantity: "Số lượng công thức",
        calculationProductQuantity: "Số lượng thành phẩm muốn sản xuất",
        noIngredientsForCalculation: "Vui lòng thêm nguyên liệu vào công thức trước khi tính toán",
        enterQuantityForCalculation: "Vui lòng nhập số lượng công thức",
        enterProductQuantityForCalculation: "Vui lòng nhập số lượng thành phẩm muốn sản xuất",
        setOutputQuantityFirst: "Vui lòng nhập số lượng thành phẩm trong tab chỉnh sửa trước",
        allIngredientsSufficient: "Tất cả nguyên liệu đủ",
        someIngredientsInsufficient: "Một số nguyên liệu không đủ",
        ingredientName: "Nguyên liệu",
        required: "Cần",
        status: "Trạng thái",
        sufficient: "Đủ",
        shortage: "Thiếu",
        requiredRecipeCount: "Số lần cần làm công thức",
        maxPossibleRecipeCount: "Số lần có thể làm công thức",
        maxPossibleProductQuantity: "Số lượng thành phẩm có thể làm được",
        tabCost: "Giá thành",
        noIngredientsForCost: "Vui lòng thêm nguyên liệu vào công thức trước khi xem giá thành",
        quantityInRecipe: "Số lượng trong công thức",
        averagePrice: "Giá trung bình",
        totalCostForQuantity: "Tổng giá cho số lượng này",
        totalRecipeCost: "Tổng giá thành công thức",
        costPerProduct: "Giá thành mỗi thành phẩm",
        for: "cho",
        products: "thành phẩm",
        noPriceData: "Chưa có dữ liệu giá",
        fromBaseRecipe: "Từ công thức nền",
        additionalIngredients: "Nguyên liệu bổ sung",
        baseRecipeCost: "Tổng giá công thức nền",
        additionalCost: "Tổng giá nguyên liệu bổ sung",
        errors: {
          nameRequired: "Tên công thức là bắt buộc",
          ingredientsRequired: "Cần ít nhất một nguyên liệu",
          quantityRequired: "Tất cả số lượng nguyên liệu phải lớn hơn 0",
          baseRecipeRequired: "Vui lòng chọn công thức nền cho công thức bánh",
          saveFailed: "Không thể lưu công thức"
        }
      }
    },
    customers: {
      title: "Khách hàng",
      pageSubtitle: "Danh bạ gắn với đơn hàng — tìm theo tên hoặc số điện thoại.",
      add: "Thêm khách hàng",
      search: "Tìm theo tên hoặc SĐT…",
      noData: "Không tìm thấy khách hàng.",
      createFirst: "Thêm khách hàng đầu tiên",
      searchEmpty: "Không có khách hàng khớp tìm kiếm.",
      listToolbarHint: "Hiển thị {{filtered}} / {{total}}",
      statsPillTotal: "Tổng {{n}} khách",
      statsPillActive: "{{n}} đã có đơn",
      namePlaceholder: "VD: Nguyễn Văn A",
      phonePlaceholder: "VD: 0901234567",
      phoneEmpty: "SĐT đang bị trống",
      phoneEmptyBadge: "Chưa có SĐT",
      loadingHint: "Đang tải danh sách khách hàng…",
      phoneBadge: {
        invalid: "SĐT chưa hợp lệ",
        viettel: "Viettel",
        vina: "VinaPhone",
        mobi: "MobiFone",
        vietnamobile: "Vietnamobile",
        gmobile: "Gmobile",
      },
      table: {
        name: "Khách hàng",
        contact: "Liên hệ",
        location: "Địa chỉ",
        totalProducts: "Tổng sản phẩm",
        actions: "Thao tác"
      },
      form: {
        addTitle: "Thêm khách hàng mới",
        editTitle: "Sửa thông tin khách hàng",
        sectionTitle: "Thông tin khách hàng",
        name: "Họ và tên",
        email: "Email",
        phone: "Số điện thoại",
        address: "Địa chỉ",
        city: "Thành phố",
        country: "Quốc gia",
        save: "Lưu khách hàng",
        errors: {
          nameRequired: "Vui lòng nhập họ tên",
          saveFailed: "Không thể lưu khách hàng",
        },
      },
      delete: {
        title: "Xóa khách hàng",
        confirm: "Bạn có chắc chắn muốn xóa khách hàng này không? Hành động này không thể hoàn tác."
      },
      detail: {
        contactSection: "Liên hệ",
        ordersHeading: "Đơn hàng ({{n}})",
        noOrders: "Chưa có đơn hàng gắn với số điện thoại này.",
        itemsLine: "{{n}} sản phẩm trong đơn",
        paymentPending: "Chưa thanh toán",
        paymentDateLabel: "Ngày thanh toán (ghi nhận)",
        paymentAtHint: "Thời điểm cập nhật khi đơn chuyển sang đã thanh toán (hệ thống không lưu riêng ngày thanh toán).",
      }
    },
    suppliers: {
      title: "Nhà cung cấp",
      add: "Thêm nhà cung cấp",
      search: "Tìm kiếm nhà cung cấp...",
      noData: "Không tìm thấy nhà cung cấp.",
      createFirst: "Thêm nhà cung cấp đầu tiên",
      form: {
        addTitle: "Thêm nhà cung cấp mới",
        editTitle: "Sửa nhà cung cấp",
        name: "Tên nhà cung cấp",
        namePlaceholder: "VD: Công ty Bột, Cty Đường",
        type: "Loại nhà cung cấp",
        types: {
          grocery: "Tạp hóa",
          tiktok: "TikTok",
          shopee: "Shopee",
          GROCERY: "Tạp hóa",
          TIKTOK: "TikTok",
          SHOPEE: "Shopee"
        },
        contactName: "Người liên hệ",
        contactPlaceholder: "Tên người liên hệ",
        phone: "Số điện thoại",
        email: "Email",
        address: "Địa chỉ",
        addressPlaceholder: "Số nhà, đường, thành phố",
        note: "Ghi chú",
        notePlaceholder: "Ghi chú thêm...",
        save: "Lưu nhà cung cấp",
        errors: {
          nameRequired: "Tên nhà cung cấp là bắt buộc",
          saveFailed: "Không thể lưu nhà cung cấp"
        }
      },
      delete: {
        title: "Xóa nhà cung cấp",
        confirm: "Bạn có chắc chắn muốn xóa nhà cung cấp này không? Hành động này không thể hoàn tác."
      },
      table: {
        name: "Nhà cung cấp",
        contact: "Liên hệ",
        actions: "Thao tác"
      }
    },
    users: {
      title: "Quản lý người dùng",
      searchPlaceholder: "Tìm theo tên, email...",
      noUsers: "Không tìm thấy người dùng nào.",
      stats: {
        total: "Tổng người dùng",
        pending: "Chờ phê duyệt",
        active: "Đã kích hoạt",
        inactive: "Đã vô hiệu hóa"
      },
      status: {
        pending: "Chờ phê duyệt",
        active: "Đã kích hoạt",
        inactive: "Đã vô hiệu hóa"
      },
      role: {
        superAdmin: "Super Admin",
        admin: "Admin",
        colaborator: "Cộng tác viên"
      },
      filter: {
        all: "Tất cả trạng thái"
      },
      table: {
        user: "Người dùng",
        customName: "Tên gợi nhớ",
        role: "Vai trò",
        status: "Trạng thái",
        lastLogin: "Đăng nhập cuối",
        actions: "Thao tác"
      },
      actions: {
        approve: "Phê duyệt",
        activate: "Kích hoạt",
        deactivate: "Vô hiệu hóa",
        edit: "Chỉnh sửa",
        cancel: "Hủy",
        save: "Lưu"
      },
      messages: {
        statusUpdated: "Cập nhật trạng thái thành công",
        customNameUpdated: "Cập nhật tên gợi nhớ thành công",
        roleUpdated: "Cập nhật vai trò thành công",
        loadError: "Không thể tải danh sách người dùng",
        updateStatusError: "Không thể cập nhật trạng thái",
        updateCustomNameError: "Không thể cập nhật tên gợi nhớ",
        updateRoleError: "Không thể cập nhật vai trò"
      }
    },
    transactions: {
      title: "Giao dịch",
      searchPlaceholder: "Tìm mã đơn, nội dung...",
      noData: "Chưa có giao dịch nào.",
      refresh: "Làm mới",
      refreshSuccess: "Đã làm mới danh sách giao dịch",
      refreshError: "Không thể làm mới danh sách giao dịch",
      loadError: "Không thể tải danh sách giao dịch",
      totalIn: "Tổng thu",
      totalOut: "Tổng chi",
      netAmount: "Số dư",
      filterAll: "Tất cả",
      filterIn: "Thu vào",
      filterOut: "Chi ra",
      results: "kết quả",
      dateRange: "Khoảng ngày",
      from: "Từ",
      to: "Đến",
      table: {
        date: "Ngày",
        amount: "Số tiền",
        orderNumber: "Mã đơn",
        content: "Nội dung",
        gateway: "Cổng TT",
        account: "Tài khoản"
      }
    },
    form: {
      createTitle: "Tạo đơn hàng mới",
      editTitle: "Chỉnh sửa đơn hàng",
      customerDetails: "Thông tin khách hàng",
      customerName: "Tên khách hàng",
      phone: "Số điện thoại",
      address: "Địa chỉ giao hàng",
      orderInfo: "Thông tin đơn hàng",
      productType: "Loại sản phẩm",
      customName: "Tên sản phẩm",
      customPlaceholder: "VD: Bánh kem đặc biệt",
      quantity: "Số lượng",
      unitPrice: "Đơn giá (VND)",
      shippingCost: "Phí vận chuyển (VND)",
      totalEstimate: "Tạm tính",
      status: "Trạng thái",
      note: "Ghi chú",
      cancel: "Hủy bỏ",
      save: "Lưu",
      edit: "Chỉnh sửa",
      saving: "Đang lưu...",
      saveProduct: "Lưu sản phẩm"
    },
    detail: {
      orderId: "Đơn hàng #",
      placedOn: "Đặt lúc",
      tabDetails: "Chi tiết đơn",
      tabAi: "Trợ lý AI",
      history: "Lịch sử",
      historyEmptyAll: "Chưa có lịch sử",
      customer: "Khách hàng",
      customerName: "Tên khách hàng",
      email: "Email",
      phone: "Số điện thoại",
      shippingAddress: "Địa chỉ nhận hàng",
      items: "Sản phẩm",
      subtotal: "Tiền hàng",
      shipping: "Phí ship",
      total: "Tổng cộng",
      fulfillment: "Vận đơn",
      tracking: "Mã vận đơn",
      payment: "Thanh toán",
      paymentMethod: "Phương thức",
      transactionNumber: "Mã giao dịch",
      notAssigned: "Chưa có",
      note: "Ghi chú",
      noNote: "Không có ghi chú",
      close: "Đóng",
      edit: "Sửa đơn",
      aiIntro: "Sử dụng AI nâng cao để phân tích đơn hàng, soạn email hoặc đánh giá rủi ro ngay lập tức.",
      draftEmail: "Soạn Email",
      riskCheck: "Kiểm tra rủi ro",
      summarize: "Tóm tắt đơn",
      copyClipboard: "Sao chép",
      consulting: "Đang hỏi Gemini..."
    },
    paymentMethod: {
      label: "Phương thức",
      cash: "Tiền mặt",
      banking: "Chuyển khoản"
    },
    qr: {
      sectionTitle: "Mã thanh toán",
      title: "Chuyển khoản (VietQR)",
      bank: "Ngân hàng",
      account: "Số TK",
      accountName: "Tên TK",
      amount: "Số tiền",
      content: "Nội dung",
      instruction: "Quét mã bằng ứng dụng ngân hàng để thanh toán."
    },
    login: {
      welcome: "Chào mừng trở lại! Vui lòng đăng nhập để tiếp tục.",
      googleButton: "Đăng nhập bằng Google",
      currentAccount: "Tài khoản hiện tại",
      useCurrentAccount: "Tiếp tục với tài khoản này",
      switchAccount: "Chọn tài khoản khác / Thêm mới",
      recentAccounts: "Tài khoản đã đăng nhập",
      accountRemoved: "Đã xóa tài khoản khỏi danh sách",
      removeAccount: "Xóa tài khoản"
    }
  }
};