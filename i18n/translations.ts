export const translations = {
  en: {
    common: {
      apply: "Apply",
      cancel: "Cancel"
    },
    nav: {
      dashboard: "Dashboard",
      orders: "Orders",
      inventory: "Inventory",
      storage: "Storage",
      customers: "Customers",
      users: "User Management",
      transactions: "Transactions",
      settings: "Settings",
      signOut: "Sign Out",
      newOrder: "New Order",
      soon: "SOON"
    },
    header: {
      dashboardTitle: "Dashboard Overview",
      ordersTitle: "Order Management",
      inventoryTitle: "Product Inventory",
      storageTitle: "Ingredient Storage",
      customersTitle: "Customer Management",
      usersTitle: "User Management",
      transactionsTitle: "Payment Transactions",
      systemOp: "System Operational"
    },
    dashboard: {
      totalRevenue: "Total Revenue",
      totalOrders: "Total Orders",
      pending: "Pending",
      avgOrderValue: "Avg. Order Value",
      revenueTrend: "Revenue Trend",
      fromLastWeek: "from last week",
      newToday: "new today",
      requiresAttention: "Requires attention",
      fromYesterday: "from yesterday",
      aiTitle: "Gemini Insights",
      analyzing: "Analyzing data...",
      updateAnalysis: "Update Analysis",
      generateBrief: "Generate Daily Brief",
      aiPlaceholder: "Generate a real-time AI analysis of your current order trends, potential risks, and operational suggestions using Google Gemini.",
      filterWeek: "Week",
      filterMonth: "Month",
      filterYear: "Year"
    },
    orders: {
      recent: "Recent Orders",
      searchPlaceholder: "Search ID or Customer...",
      filterProductPlaceholder: "Filter product...",
      filters: "Filters",
      fromDate: "From date",
      toDate: "To date",
      startDate: "Start Month",
      endDate: "End Month",
      allMonths: "All Months",
      allStatus: "All Status",
      noOrders: "No orders found.",
      noOrdersCriteria: "No orders found matching your criteria.",
      createFirst: "Create First Order",
      showing: "Showing",
      of: "of",
      viewDetails: "View Details",
      tableId: "ID",
      tableDate: "Date",
      tableDeliveryDate: "Delivery date",
      tableCreatedUpdated: "Created / Updated",
      tableCustomer: "Customer",
      tableProduct: "Product",
      tableStatus: "Status",
      tableTotal: "Total",
      tableActions: "Actions",
      labelCreated: "Created",
      labelUpdated: "Updated",
      statusLabels: {
        PENDING: "Pending",
        PROCESSING: "Processing",
        DELIVERED: "Delivered",
        CANCELLED: "Cancelled",
        RETURNED: "Returned"
      },
      paymentStatusLabels: {
        PAID: "Paid",
        UNPAID: "Unpaid",
        REFUNDED: "Refunded"
      },
      delete: "Delete Order",
      confirmDelete: "Are you sure you want to delete this order? This action cannot be undone.",
      exportCsv: "Export CSV",
      exportTitle: "Export Order Data",
      exportCurrentMonth: "Current Month",
      exportMonth: "Select Month",
      exportAllTime: "All Time",
      exportCustom: "Custom Range",
      exportStart: "Start Date",
      exportEnd: "End Date",
      exportCancel: "Cancel",
      exportConfirm: "Export"
    },
    inventory: {
      title: "Products",
      addProduct: "Add Product",
      searchPlaceholder: "Search products...",
      noProducts: "No products found.",
      createFirst: "Add your first product",
      formTitleAdd: "Add New Product",
      formTitleEdit: "Edit Product",
      name: "Product Name",
      price: "Price (VND)",
      category: "Category",
      image: "Image URL",
      description: "Description",
      status: "Status",
      active: "Active",
      inactive: "Inactive",
      deleteConfirm: "Are you sure you want to delete this product?"
    },
    storage: {
      title: "Ingredients",
      addIngredient: "Add Ingredient",
      exportExcel: "Export Ingredients Excel",
      searchPlaceholder: "Search ingredients...",
      noIngredients: "No ingredients found.",
      createFirst: "Add your first ingredient",
      formTitleAdd: "Add New Ingredient",
      formTitleEdit: "Edit Ingredient",
      name: "Ingredient Name",
      namePlaceholder: "e.g. Flour, Sugar, Milk",
      quantity: "Quantity",
      unit: "Unit",
      price: "Cost per Unit (VND)",
      supplier: "Supplier",
      supplierPlaceholder: "Supplier name",
      note: "Note",
      notePlaceholder: "Additional notes...",
      lastUpdated: "Last Updated",
      totalValue: "Total Value",
      totalIngredients: "Total Ingredients",
      lowStock: "Low Stock Items",
      lowStockBadge: "Low stock!",
      lowStockTooltip: "Low stock",
      currentQuantityLabel: "Current",
      weight: "Weight",
      exampleWeight: "Example: 10kg, 5l",
      unitQuantity: "Unit quantity",
      exampleUnitQuantity: "Example: 10 boxes, 5 cartons",
      columnQuantity: "Quantity",
      columnImportCount: "Imports",
      columnItemValue: "Total Value",
      columnLastImport: "Last Import",
      columnActions: "Actions",
      actionViewHistory: "View history",
      actionImport: "Add import",
      actionInventory: "Inventory count",
      actionEdit: "Edit",
      actionDelete: "Delete",
      // Import
      importTitle: "Add Import",
      importPrice: "Import Price",
      importPriceWithUnit: "Import Price (VND)",
      importDateTime: "Import Date & Time",
      saveImportVoucher: "Save Import Voucher",
      // Inventory
      inventoryTitle: "Inventory Count",
      currentQuantityCalculated: "Current quantity (calculated)",
      remainingWeight: "Remaining weight",
      inventoryDateTime: "Inventory Date & Time",
      saveInventoryVoucher: "Save Inventory Voucher",
      // History modal
      historyTitle: "Import & Inventory History",
      historyTabAll: "All",
      historyTabImport: "Imports",
      historyTabInventory: "Inventories",
      historyEmptyAll: "No history yet",
      historyEmptyImport: "No import history yet",
      historyEmptyInventory: "No inventory history yet",
      historyTotalPrice: "Total amount",
      // Low stock modal
      lowStockModalTitle: "Low Stock Ingredients",
      lowStockModalSubtitle: "ingredients need restocking",
      percentageOfImported: "% of total imported",
      totalImported: "Total imported:",
      averagePrice: "Average price:",
      lastImport: "Last import:",
      noLowStockItems: "No ingredients are low on stock",
      close: "Close",
      deleteConfirm: "Are you sure you want to delete this ingredient?",
      // Error messages
      errors: {
        nameRequired: "Ingredient name is required",
        supplierRequired: "Supplier is required",
        duplicateIngredient: "An ingredient with this name and supplier already exists",
        saveIngredient: "Unable to save ingredient",
        importAmountPositive: "Weight must be greater than 0",
        importPriceNonNegative: "Price cannot be negative",
        importDateRequired: "Import date is required",
        importSaveError: "Unable to save import voucher",
        inventoryAmountNonNegative: "Weight cannot be negative",
        inventoryQuantityNonNegative: "Quantity cannot be negative",
        inventoryDateRequired: "Inventory date is required",
        inventorySaveError: "Unable to save inventory voucher"
      }
    },
    customers: {
      title: "Customers",
      add: "Add Customer",
      search: "Search customers...",
      noData: "No customers found.",
      createFirst: "Add your first customer",
      table: {
        name: "Customer",
        contact: "Contact Info",
        location: "Location",
        totalProducts: "Total Products",
        actions: "Actions"
      },
      form: {
        addTitle: "Add New Customer",
        editTitle: "Edit Customer",
        name: "Full Name",
        email: "Email",
        phone: "Phone Number",
        address: "Address",
        city: "City",
        country: "Country",
        save: "Save Customer"
      },
      delete: {
        title: "Delete Customer",
        confirm: "Are you sure you want to delete this customer? This action cannot be undone."
      }
    },
    users: {
      title: "User Management",
      searchPlaceholder: "Search by name, email...",
      noUsers: "No users found.",
      stats: {
        total: "Total Users",
        pending: "Pending",
        active: "Active",
        inactive: "Inactive"
      },
      status: {
        pending: "Pending",
        active: "Active",
        inactive: "Inactive"
      },
      role: {
        superAdmin: "Super Admin",
        admin: "Admin",
        colaborator: "Colaborator"
      },
      filter: {
        all: "All Status"
      },
      table: {
        user: "User",
        customName: "Custom Name",
        role: "Role",
        status: "Status",
        lastLogin: "Last Login",
        actions: "Actions"
      },
      actions: {
        approve: "Approve",
        activate: "Activate",
        deactivate: "Deactivate",
        edit: "Edit",
        cancel: "Cancel",
        save: "Save"
      },
      messages: {
        statusUpdated: "User status updated successfully",
        customNameUpdated: "Custom name updated successfully",
        roleUpdated: "User role updated successfully",
        loadError: "Failed to load users",
        updateStatusError: "Failed to update status",
        updateCustomNameError: "Failed to update custom name",
        updateRoleError: "Failed to update role"
      }
    },
    transactions: {
      title: "Transactions",
      searchPlaceholder: "Search order #, content...",
      noData: "No transactions found.",
      table: {
        date: "Date",
        amount: "Amount",
        orderNumber: "Order Ref",
        content: "Content",
        gateway: "Gateway",
        account: "Account"
      }
    },
    form: {
      createTitle: "Create New Order",
      editTitle: "Edit Order",
      customerDetails: "Customer Details",
      customerName: "Customer Name",
      phone: "Phone Number",
      address: "Delivery Address",
      orderInfo: "Order Info",
      productType: "Product Type",
      customName: "Tên sản phẩm",
      customPlaceholder: "e.g. Special Cake",
      quantity: "Quantity",
      unitPrice: "Unit Price (VND)",
      shippingCost: "Shipping Cost (VND)",
      totalEstimate: "Total Estimate",
      status: "Status",
      note: "Note",
      cancel: "Cancel",
      save: "Save Order",
      saving: "Saving...",
      saveProduct: "Save Product",
      saveIngredient: "Save Ingredient"
    },
    detail: {
      orderId: "Order #",
      placedOn: "Placed on",
      tabDetails: "Order Details",
      tabAi: "AI Assistant",
      customer: "Customer",
      shippingAddress: "Shipping Address",
      items: "Items",
      subtotal: "Subtotal",
      shipping: "Shipping",
      total: "Total",
      fulfillment: "Fulfillment",
      tracking: "Tracking Number",
      payment: "Payment Status",
      paymentMethod: "Payment Method",
      transactionNumber: "Transaction Number",
      notAssigned: "Not assigned",
      note: "Note",
      noNote: "No note provided",
      close: "Close",
      edit: "Edit Order",
      aiIntro: "Use advanced AI to analyze this order, draft communications, or assess risk factors instantly.",
      draftEmail: "Draft Email",
      riskCheck: "Risk Check",
      summarize: "Summarize",
      copyClipboard: "Copy to Clipboard",
      consulting: "Consulting Gemini..."
    },
    paymentMethod: {
      label: "Payment Method",
      cash: "Cash",
      banking: "Banking"
    },
    qr: {
      sectionTitle: "Payment QR",
      title: "Bank Transfer (VietQR)",
      bank: "Bank",
      account: "Account",
      accountName: "Name",
      amount: "Amount",
      content: "Content",
      instruction: "Scan with any banking app to pay."
    },
    login: {
      welcome: "Welcome back! Please sign in to continue.",
      googleButton: "Sign in with Google",
      currentAccount: "Current Account",
      useCurrentAccount: "Continue with this account",
      switchAccount: "Switch Account / Add New",
      recentAccounts: "Recent Accounts",
      accountRemoved: "Account removed from history",
      removeAccount: "Remove account"
    },
    units: {
      kg: "kg",
      g: "g",
      l: "l",
      ml: "ml",
      pcs: "pcs",
      box: "box",
      can: "can",
      bottle: "bottle"
    }
  },
  vi: {
    common: {
      apply: "Áp dụng",
      cancel: "Hủy"
    },
    nav: {
      dashboard: "Tổng quan",
      orders: "Đơn hàng",
      inventory: "Kho hàng",
      storage: "Nguyên liệu",
      customers: "Khách hàng",
      users: "Quản lý người dùng",
      transactions: "Giao dịch",
      settings: "Cài đặt",
      signOut: "Đăng xuất",
      newOrder: "Tạo đơn",
      soon: "SẮP RA"
    },
    header: {
      dashboardTitle: "Tổng quan hệ thống",
      ordersTitle: "Quản lý đơn hàng",
      inventoryTitle: "Kho sản phẩm",
      storageTitle: "Kho nguyên liệu",
      customersTitle: "Quản lý khách hàng",
      transactionsTitle: "Lịch sử giao dịch",
      usersTitle: "Quản lý người dùng",
      systemOp: "Hệ thống ổn định"
    },
    dashboard: {
      totalRevenue: "Tổng doanh thu",
      totalOrders: "Tổng đơn hàng",
      pending: "Chờ xử lý",
      avgOrderValue: "Giá trị TB",
      revenueTrend: "Biểu đồ doanh thu",
      fromLastWeek: "so với tuần trước",
      newToday: "đơn mới hôm nay",
      requiresAttention: "Cần chú ý",
      fromYesterday: "so với hôm qua",
      aiTitle: "Phân tích Gemini",
      analyzing: "Đang phân tích...",
      updateAnalysis: "Cập nhật phân tích",
      generateBrief: "Tạo báo cáo ngày",
      aiPlaceholder: "Sử dụng AI để phân tích xu hướng bán hàng, rủi ro tiềm ẩn và đề xuất vận hành theo thời gian thực.",
      filterWeek: "Tuần",
      filterMonth: "Tháng",
      filterYear: "Năm"
    },
    orders: {
      recent: "Đơn hàng gần đây",
      searchPlaceholder: "Tìm ID hoặc Khách hàng...",
      filterProductPlaceholder: "Lọc theo sản phẩm...",
      filters: "Bộ lọc",
      fromDate: "Từ ngày",
      toDate: "Đến ngày",
      startDate: "Tháng bắt đầu",
      endDate: "Tháng kết thúc",
      allMonths: "Tất cả các tháng",
      allStatus: "Tất cả trạng thái",
      noOrders: "Chưa có đơn hàng nào.",
      noOrdersCriteria: "Không tìm thấy đơn hàng phù hợp.",
      createFirst: "Tạo đơn đầu tiên",
      showing: "Hiển thị",
      of: "trên",
      viewDetails: "Xem chi tiết",
      tableId: "ID",
      tableDate: "Ngày",
      tableDeliveryDate: "Ngày giao",
      tableCreatedUpdated: "Tạo / Sửa",
      tableCustomer: "Khách hàng",
      tableProduct: "Sản phẩm",
      tableStatus: "Trạng thái",
      tableTotal: "Tổng tiền",
      tableActions: "Thao tác",
      labelCreated: "Tạo",
      labelUpdated: "Sửa",
      statusLabels: {
        PENDING: "Chờ xử lý",
        PROCESSING: "Đang xử lý",
        DELIVERED: "Đã giao",
        CANCELLED: "Đã hủy",
        RETURNED: "Trả hàng"
      },
      paymentStatusLabels: {
        PAID: "Đã thanh toán",
        UNPAID: "Chưa thanh toán",
        REFUNDED: "Đã hoàn trả"
      },
      delete: "Xóa đơn hàng",
      confirmDelete: "Bạn có chắc chắn muốn xóa đơn hàng này không? Hành động này không thể hoàn tác.",
      exportCsv: "Xuất CSV",
      exportTitle: "Xuất dữ liệu đơn hàng",
      exportCurrentMonth: "Tháng này",
      exportMonth: "Chọn tháng",
      exportAllTime: "Tất cả",
      exportCustom: "Tùy chỉnh",
      exportStart: "Từ ngày",
      exportEnd: "Đến ngày",
      exportCancel: "Hủy",
      exportConfirm: "Xuất file"
    },
    inventory: {
      title: "Sản phẩm",
      addProduct: "Thêm sản phẩm",
      searchPlaceholder: "Tìm kiếm sản phẩm...",
      noProducts: "Chưa có sản phẩm nào.",
      createFirst: "Thêm sản phẩm đầu tiên",
      formTitleAdd: "Thêm sản phẩm mới",
      formTitleEdit: "Sửa sản phẩm",
      name: "Tên sản phẩm",
      price: "Giá (VND)",
      category: "Danh mục",
      image: "Link ảnh",
      description: "Mô tả",
      status: "Trạng thái",
      active: "Hoạt động",
      inactive: "Ngừng bán",
      deleteConfirm: "Bạn có chắc chắn muốn xóa sản phẩm này không?"
    },
    storage: {
      title: "Nguyên liệu",
      addIngredient: "Thêm nguyên liệu",
      exportExcel: "Xuất Excel nguyên liệu",
      searchPlaceholder: "Tìm nguyên liệu...",
      noIngredients: "Chưa có nguyên liệu nào.",
      createFirst: "Thêm nguyên liệu đầu tiên",
      formTitleAdd: "Thêm nguyên liệu mới",
      formTitleEdit: "Sửa nguyên liệu",
      name: "Tên nguyên liệu",
      namePlaceholder: "Ví dụ: Bột mì, Đường, Sữa",
      quantity: "Số lượng",
      unit: "Đơn vị",
      price: "Đơn giá (VND)",
      supplier: "Nhà cung cấp",
      supplierPlaceholder: "Tên nhà cung cấp",
      note: "Ghi chú",
      notePlaceholder: "Ghi chú thêm...",
      lastUpdated: "Cập nhật lần cuối",
      totalValue: "Tổng giá trị",
      totalIngredients: "Tổng số nguyên liệu",
      lowStock: "Sắp hết hàng",
      lowStockBadge: "Sắp hết!",
      lowStockTooltip: "Sắp hết",
      currentQuantityLabel: "Hiện tại",
      weight: "Khối lượng",
      exampleWeight: "Ví dụ: 10kg, 5l",
      unitQuantity: "Số lượng đơn vị định lượng",
      exampleUnitQuantity: "Ví dụ: 10 hộp, 5 thùng",
      columnQuantity: "Số lượng",
      columnImportCount: "Số lần nhập",
      columnItemValue: "Thành tiền",
      columnLastImport: "Nhập gần nhất",
      columnActions: "Thao tác",
      actionViewHistory: "Xem lịch sử",
      actionImport: "Nhập hàng",
      actionInventory: "Kiểm kê",
      actionEdit: "Chỉnh sửa",
      actionDelete: "Xóa",
      // Import
      importTitle: "Nhập hàng",
      importPrice: "Giá nhập",
      importPriceWithUnit: "Giá nhập (VND)",
      importDateTime: "Ngày giờ nhập hàng",
      saveImportVoucher: "Lưu phiếu nhập",
      // Inventory
      inventoryTitle: "Kiểm kê",
      currentQuantityCalculated: "Số lượng hiện tại (tính toán)",
      remainingWeight: "Khối lượng còn lại",
      inventoryDateTime: "Ngày giờ kiểm kê",
      saveInventoryVoucher: "Lưu phiếu kiểm kê",
      // History modal
      historyTitle: "Lịch sử nhập xuất",
      historyTabAll: "Tất cả",
      historyTabImport: "Nhập hàng",
      historyTabInventory: "Kiểm kê",
      historyEmptyAll: "Chưa có lịch sử nhập xuất",
      historyEmptyImport: "Chưa có lịch sử nhập hàng",
      historyEmptyInventory: "Chưa có lịch sử kiểm kê",
      historyTotalPrice: "Thành tiền",
      // Low stock modal
      lowStockModalTitle: "Nguyên liệu sắp hết",
      lowStockModalSubtitle: "nguyên liệu cần nhập thêm",
      percentageOfImported: "% so với đã nhập",
      totalImported: "Tổng đã nhập:",
      averagePrice: "Giá trung bình:",
      lastImport: "Nhập gần nhất:",
      noLowStockItems: "Không có nguyên liệu nào sắp hết",
      close: "Đóng",
      deleteConfirm: "Bạn có chắc chắn muốn xóa nguyên liệu này không?",
      // Error messages
      errors: {
        nameRequired: "Tên nguyên liệu là bắt buộc",
        supplierRequired: "Nhà cung cấp là bắt buộc",
        duplicateIngredient: "Nguyên liệu với tên và nhà cung cấp này đã tồn tại",
        saveIngredient: "Không thể lưu nguyên liệu",
        importAmountPositive: "Khối lượng phải lớn hơn 0",
        importPriceNonNegative: "Giá không thể âm",
        importDateRequired: "Ngày nhập hàng là bắt buộc",
        importSaveError: "Không thể lưu phiếu nhập hàng",
        inventoryAmountNonNegative: "Khối lượng không thể âm",
        inventoryQuantityNonNegative: "Số lượng không thể âm",
        inventoryDateRequired: "Ngày kiểm kê là bắt buộc",
        inventorySaveError: "Không thể lưu phiếu kiểm kê"
      }
    },
    customers: {
      title: "Khách hàng",
      add: "Thêm khách hàng",
      search: "Tìm kiếm khách hàng...",
      noData: "Không tìm thấy khách hàng.",
      createFirst: "Thêm khách hàng đầu tiên",
      table: {
        name: "Khách hàng",
        contact: "Liên hệ",
        location: "Địa chỉ",
        totalProducts: "Tổng sản phẩm",
        actions: "Thao tác"
      },
      form: {
        addTitle: "Thêm khách hàng mới",
        editTitle: "Sửa thông tin khách hàng",
        name: "Họ và tên",
        email: "Email",
        phone: "Số điện thoại",
        address: "Địa chỉ",
        city: "Thành phố",
        country: "Quốc gia",
        save: "Lưu khách hàng"
      },
      delete: {
        title: "Xóa khách hàng",
        confirm: "Bạn có chắc chắn muốn xóa khách hàng này không? Hành động này không thể hoàn tác."
      }
    },
    users: {
      title: "Quản lý người dùng",
      searchPlaceholder: "Tìm theo tên, email...",
      noUsers: "Không tìm thấy người dùng nào.",
      stats: {
        total: "Tổng người dùng",
        pending: "Chờ phê duyệt",
        active: "Đã kích hoạt",
        inactive: "Đã vô hiệu hóa"
      },
      status: {
        pending: "Chờ phê duyệt",
        active: "Đã kích hoạt",
        inactive: "Đã vô hiệu hóa"
      },
      role: {
        superAdmin: "Super Admin",
        admin: "Admin",
        colaborator: "Cộng tác viên"
      },
      filter: {
        all: "Tất cả trạng thái"
      },
      table: {
        user: "Người dùng",
        customName: "Tên gợi nhớ",
        role: "Vai trò",
        status: "Trạng thái",
        lastLogin: "Đăng nhập cuối",
        actions: "Thao tác"
      },
      actions: {
        approve: "Phê duyệt",
        activate: "Kích hoạt",
        deactivate: "Vô hiệu hóa",
        edit: "Chỉnh sửa",
        cancel: "Hủy",
        save: "Lưu"
      },
      messages: {
        statusUpdated: "Cập nhật trạng thái thành công",
        customNameUpdated: "Cập nhật tên gợi nhớ thành công",
        roleUpdated: "Cập nhật vai trò thành công",
        loadError: "Không thể tải danh sách người dùng",
        updateStatusError: "Không thể cập nhật trạng thái",
        updateCustomNameError: "Không thể cập nhật tên gợi nhớ",
        updateRoleError: "Không thể cập nhật vai trò"
      }
    },
    transactions: {
      title: "Giao dịch",
      searchPlaceholder: "Tìm mã đơn, nội dung...",
      noData: "Chưa có giao dịch nào.",
      table: {
        date: "Ngày",
        amount: "Số tiền",
        orderNumber: "Mã đơn",
        content: "Nội dung",
        gateway: "Cổng TT",
        account: "Tài khoản"
      }
    },
    form: {
      createTitle: "Tạo đơn hàng mới",
      editTitle: "Chỉnh sửa đơn hàng",
      customerDetails: "Thông tin khách hàng",
      customerName: "Tên khách hàng",
      phone: "Số điện thoại",
      address: "Địa chỉ giao hàng",
      orderInfo: "Thông tin đơn hàng",
      productType: "Loại sản phẩm",
      customName: "Tên sản phẩm",
      customPlaceholder: "VD: Bánh kem đặc biệt",
      quantity: "Số lượng",
      unitPrice: "Đơn giá (VND)",
      shippingCost: "Phí vận chuyển (VND)",
      totalEstimate: "Tạm tính",
      status: "Trạng thái",
      note: "Ghi chú",
      cancel: "Hủy bỏ",
      save: "Lưu đơn hàng",
      saving: "Đang lưu...",
      saveProduct: "Lưu sản phẩm",
      saveIngredient: "Lưu nguyên liệu"
    },
    detail: {
      orderId: "Đơn hàng #",
      placedOn: "Đặt lúc",
      tabDetails: "Chi tiết đơn",
      tabAi: "Trợ lý AI",
      customer: "Khách hàng",
      shippingAddress: "Địa chỉ nhận hàng",
      items: "Sản phẩm",
      subtotal: "Tiền hàng",
      shipping: "Phí ship",
      total: "Tổng cộng",
      fulfillment: "Vận đơn",
      tracking: "Mã vận đơn",
      payment: "Thanh toán",
      paymentMethod: "Phương thức",
      transactionNumber: "Mã giao dịch",
      notAssigned: "Chưa có",
      note: "Ghi chú",
      noNote: "Không có ghi chú",
      close: "Đóng",
      edit: "Sửa đơn",
      aiIntro: "Sử dụng AI nâng cao để phân tích đơn hàng, soạn email hoặc đánh giá rủi ro ngay lập tức.",
      draftEmail: "Soạn Email",
      riskCheck: "Kiểm tra rủi ro",
      summarize: "Tóm tắt đơn",
      copyClipboard: "Sao chép",
      consulting: "Đang hỏi Gemini..."
    },
    paymentMethod: {
      label: "Phương thức",
      cash: "Tiền mặt",
      banking: "Chuyển khoản"
    },
    qr: {
      sectionTitle: "Mã thanh toán",
      title: "Chuyển khoản (VietQR)",
      bank: "Ngân hàng",
      account: "Số TK",
      accountName: "Tên TK",
      amount: "Số tiền",
      content: "Nội dung",
      instruction: "Quét mã bằng ứng dụng ngân hàng để thanh toán."
    },
    login: {
      welcome: "Chào mừng trở lại! Vui lòng đăng nhập để tiếp tục.",
      googleButton: "Đăng nhập bằng Google",
      currentAccount: "Tài khoản hiện tại",
      useCurrentAccount: "Tiếp tục với tài khoản này",
      switchAccount: "Chọn tài khoản khác / Thêm mới",
      recentAccounts: "Tài khoản đã đăng nhập",
      accountRemoved: "Đã xóa tài khoản khỏi danh sách",
      removeAccount: "Xóa tài khoản"
    },
    units: {
      kg: "kg",
      g: "g",
      l: "l",
      ml: "ml",
      pcs: "cái",
      box: "hộp",
      can: "lon",
      bottle: "chai"
    }
  }
};