...
#
# Dùng:
#   ./deploy.sh              # build amd64 + push :staging và :<sha> → rollout deploy/frontend
...
VITE_API_URL="https://api-staging.cucquy.site/api"
VPS="rice@ssh.ricevps.xyz"
NS="cucquy-staging"; DEPLOY="frontend"
DEPLOY_DIR="~/deploys/cucquy-frontend-staging"
...
TAGS=(-t "$IMAGE:$SHA"); [ "$PUSH_LATEST" -eq 1 ] && TAGS+=(-t "$IMAGE:staging")
