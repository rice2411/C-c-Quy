...
import {
  ...
  calculateRequiredRecipeCount,
  calculateMaxPossibleProductQuantity,
} from "@/utils/recipeUtil";
...
const FullRecipeForm: React.FC<FullRecipeFormProps> = ({
  isOpen,
  initialData,
  ingredients,
  onSave,
  ...
  const [quantityModalIngredient, setQuantityModalIngredient] = useState<{
  ...
  useEffect(() => {
    const loadBaseRecipes = async () => {
      try {
        ...
        const baseOnly = recipes.filter(
          (r) => r.recipeType === "base" || (!r.recipeType && !r.baseRecipeId)
        );
    ...
    if (baseRecipeId) {
      ...
      if (selectedBase) {
        ...
        const currentFlavorTopping = recipeIngredients.filter((ri) => {
          ...
          return (
            ing &&
            (ing.type === IngredientType.FLAVOR ||
              ing.type === IngredientType.TOPPING)
          );
        });
  ...
  const handleToggleIngredient = (ingredient: Ingredient) => {
    ...
    if (isSelected) {
      setRecipeIngredients(
        recipeIngredients.filter((ri) => ri.ingredientId !== ingredient.id)
      );
    } else {
    ...
    if (!isNaN(numValue) && numValue >= 0) {
      setRecipeIngredients(
        recipeIngredients.map((ri) =>
          ri.ingredientId === quantityModalIngredient.ingredient.id
            ? { ...ri, quantity: numValue }
            : ri
        )
      );
    }
  ...
  const handleCancelQuantityModal = () => {
    ...
    const selected = getSelectedIngredient(
      quantityModalIngredient.ingredient.id
    );
    if (selected && selected.quantity === 0) {
      setRecipeIngredients(
        recipeIngredients.filter(
          (ri) => ri.ingredientId !== quantityModalIngredient.ingredient.id
        )
      );
    }
    setShowQuantityModal(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (recipeIngredients.length === 0)
        throw new Error(t("recipes.form.errors.ingredientsRequired"));
  ...
  const calculationResults = useMemo(() => {
    ...
    return calculateIngredientRequirements(
      recipeIngredients,
      ingredients,
      requiredRecipeCount
    );
  ...
  const calculatedRecipeCount = useMemo(() => {
    ...
    return calculateRequiredRecipeCount(
      outputQuantity,
      outputQuantity,
      wasteRate
    );
  ...
  const maxPossibleCalculation = useMemo(() => {
    return calculateMaxPossibleProductQuantity(
      recipeIngredients,
      ingredients,
      outputQuantity
    );
  ...
  const headerContent = (
    <div className="flex-1 min-w-0 pr-2">
      <h2 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white">
        {initialData
          ? t("recipes.form.editFullRecipe") || "Sửa công thức bánh"
          : t("recipes.form.addFullRecipe") || "Thêm công thức bánh"}
      </h2>
      <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
        {t("recipes.form.fullRecipeSubtitle") ||
          "Tạo công thức bánh từ công thức nền + hương vị + topping"}
      </p>
    </div>
  );
      ...
      <div className="flex-1 overflow-y-auto bg-slate-50/50 dark:bg-slate-900/50 p-4 sm:p-6 space-y-4 sm:space-y-5">
        ...
        {activeTab === "edit" ? (
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
            <div className="bg-white dark:bg-slate-800 p-3 sm:p-4 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm space-y-3">
              <div>
                ...
                <div className="relative">
                  ...
                  >
                    <option value="">
                      {t("recipes.form.selectBaseRecipe")}
                    </option>
            ...
            {selectedBaseRecipe &&
              selectedBaseRecipe.ingredients.length > 0 && (
                <div className="bg-white dark:bg-slate-800 p-3 sm:p-4 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm space-y-3">
            ...
            <div className="bg-white dark:bg-slate-800 p-3 sm:p-4 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="text-xs sm:text-sm font-semibold text-slate-900 dark:text-white uppercase tracking-wide">
                  ...
                  {t("ingredients.form.types.flavor")} +{" "}
                  {t("ingredients.form.types.topping")})
                </h3>
                <span className="text-xs text-slate-500 dark:text-slate-400">
                  ...
                  {t("recipes.form.selected") || "đã chọn"}
                </span>
              </div>
                        ...
                        {isSelected && selectedIngredient && (
                          ...
                          >
                            {selectedIngredient.quantity > 0
                              ? `${selectedIngredient.quantity.toLocaleString()} ${
                                  selectedIngredient.unit === "piece"
                                    ? t("ingredients.form.unitPiece")
                                    : "g"
                                }`
                              : t("recipes.form.setQuantity") ||
                                "Nhập số lượng"}
                          </div>
                        )}
                      </div>
                ...
                <div className="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/30 dark:to-blue-800/20 p-4 rounded-xl border border-blue-200 dark:border-blue-800 shadow-sm">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs font-semibold text-blue-700 dark:text-blue-300 uppercase tracking-wide">
                      {t("recipes.form.outputQuantityOfRecipe") ||
                        "Số lượng thành phẩm của công thức"}
                    </span>
                  </div>
                    ...
                    <div className="bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-700/50 p-4 rounded-xl border border-slate-200 dark:border-slate-600 shadow-sm">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wide">
                          {t("recipes.form.maxPossibleRecipeCount") ||
                            "Số lần có thể làm công thức"}
                        </span>
                      </div>
                    ...
                    <div className="bg-gradient-to-br from-orange-50 to-orange-100 dark:from-orange-900/30 dark:to-orange-800/20 p-4 rounded-xl border-2 border-orange-300 dark:border-orange-700 shadow-lg sm:col-span-2 lg:col-span-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-xs font-semibold text-orange-700 dark:text-orange-300 uppercase tracking-wide">
                          {t("recipes.form.maxPossibleProductQuantity") ||
                            "Số lượng thành phẩm có thể làm được"}
                        </span>
                      </div>
            ...
            {!outputQuantity || outputQuantity <= 0 ? (
              <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm text-center">
                <p className="text-slate-500 dark:text-slate-400 text-sm">
                  {t("recipes.form.setOutputQuantityFirst") ||
                    "Vui lòng nhập số lượng thành phẩm trong tab chỉnh sửa trước"}
                </p>
              </div>
            ) : recipeIngredients.length === 0 ? (
              <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm text-center">
                <p className="text-slate-500 dark:text-slate-400 text-sm">
                  {t("recipes.form.noIngredientsForCalculation") ||
                    "Vui lòng thêm nguyên liệu vào công thức trước khi tính toán"}
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                ...
                >
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    >
                      {allSufficient
                        ? t("recipes.form.allIngredientsSufficient") ||
                          "Tất cả nguyên liệu đủ"
                        : t("recipes.form.someIngredientsInsufficient") ||
                                ...
                                <div className="flex items-center gap-2">
                                  <div
                                    className={`p-1.5 rounded-lg ${
                                      getTypeColors(ingredient.type).bg
                                    } ${
                                      getTypeColors(ingredient.type).border
                                    } border`}
                                  >
                                    <TypeIcon
                                      className={`w-3.5 h-3.5 ${
                                        getTypeColors(ingredient.type).icon
                                      }`}
                                    />
                                  </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    min="0"
                    step={
                      quantityModalIngredient.ingredient.unit === "piece"
                        ? 1
                        : 0.01
                    }
