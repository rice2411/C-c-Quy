...
import { Truck, Package, Wallet, Clock, AlertTriangle, MapPin, CheckCircle2 } from 'lucide-react';
import {
  ShippingAnalytics,
  ShippingRange,
  CarrierStat,
    ...
    <Box layoutClassName="flex h-full flex-col gap-4 overflow-y-auto p-4">
      <Box layoutClassName="flex flex-wrap items-center justify-between gap-3">
        <Heading level={1} layoutClassName="flex items-center gap-2" textClassName="text-xl font-bold">
          <Truck className="h-5 w-5 text-cyan-500" /> Vận chuyển (theo đơn vị vận chuyển)
        </Heading>
        <Select
          size="sm"
          value={preset}
      ...
      {loading ? (
        ...
        <>
          {/* 1 + 2 + 3: mỗi DVVC 1 thẻ (số đơn/doanh thu, thời gian giao + histogram, trạng thái) */}
