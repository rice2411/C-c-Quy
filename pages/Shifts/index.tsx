...
const ShiftsPage: React.FC = () => {
  ...
  const dirty = useMemo(
    () => JSON.stringify(draft) !== JSON.stringify(shifts),
    [draft, shifts],
  );
  ...
  const toggleDay = (code: string, iso: number) =>
    setDraft((prev) =>
      prev.map((s) => {
        ...
        const weekdays = has
          ? s.weekdays.filter((d) => d !== iso)
          : [...s.weekdays, iso].sort((a, b) => a - b);
        return { ...s, weekdays };
      }),
    );
        ...
        <Box layoutClassName="flex items-center gap-2">
          ...
          <Box layoutClassName="flex flex-col">
            ...
            <Typography size="xs" textClassName="text-slate-400 dark:text-slate-500">
              Giờ làm + thứ trong tuần áp dụng cho mỗi ca (lặp hằng tuần). Dùng để sinh ca trên màn Lịch.
            </Typography>
          </Box>
        </Box>
        <Button
            ...
            return (
              ...
              >
                ...
                <Box layoutClassName="flex items-center gap-2">
                  ...
                  <Input
                    value={s.name}
                    onChange={(e) => patch(s.code, { name: e.target.value })}
                    placeholder="Tên ca"
                ...
                <Box layoutClassName="grid grid-cols-2 gap-3">
                  <Field label="Bắt đầu" htmlFor={`start-${s.code}`}>
                    <Input
                      ...
                      type="time"
                      value={s.startTime}
                      onChange={(e) => patch(s.code, { startTime: e.target.value })}
                    />
                  </Field>
                  <Field label="Kết thúc" htmlFor={`end-${s.code}`}>
                    <Input
                      ...
                      type="time"
                      value={s.endTime}
                      onChange={(e) => patch(s.code, { endTime: e.target.value })}
                    />
                  </Field>
                </Box>
                ...
                {/* Công/ca */}
                <Field label="Công / ca" htmlFor={`cong-${s.code}`}>
                  <Input
                    id={`cong-${s.code}`}
                    type="number"
                    value={String(s.congFactor)}
                    onChange={(e) => patch(s.code, { congFactor: Number(e.target.value) || 0 })}
                    placeholder="0.5"
