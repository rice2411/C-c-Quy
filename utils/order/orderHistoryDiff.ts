import { OrderFieldChange, OrderItem } from '@/types';

/**
 * Helper format giá trị cho việc so sánh + hiển thị
 */
...
/** Cau hinh field đuoc theo doi trong history */
const TRACKED_FIELDS: Array<{
  key: string;
  label: string;
  /** Custom getter — neu khong co dung obj[key] */
  get?: (o: any) => any;
  /** Custom comparator — neu khong co dung === */
  ...
  { key: 'note', label: 'Ghi chú' },
  {
    key: 'customer.name',
    label: 'Tên khách',
    get: (o) => o?.customer?.name,
  },
  {
    key: 'customer.phone',
    label: 'SĐT khách',
    get: (o) => o?.customer?.phone,
  },
  {
    key: 'customer.address',
    label: 'Địa chỉ',
    get: (o) => o?.customer?.address,
  },
  {
    key: 'items',
    label: 'Sản phẩm',
...
export const diffOrders = (prev: any, next: any): OrderFieldChange[] => {
  ...
  for (const cfg of TRACKED_FIELDS) {
    ...
    if (eq(a, b)) continue;
    // Ignore changes giữa null/undefined/'' (coi như khong doi)
